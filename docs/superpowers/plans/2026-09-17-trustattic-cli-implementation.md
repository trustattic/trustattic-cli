...
// the leading tag word, any HTTP-method words, and any word that is a
// tag-prefixed path parameter's own suffix leaking into the operationId
// (ProjectSlugGet: "Slug" comes from project_slug, whose first token
// "Project" equals the "project" tag). This only strips the suffix of a
// param named "<tag>_<suffix>" - stripping on *any* word overlap with *any*
// param would be too broad: e.g. BackupRestoreGet's "Restore" must survive
// even though it happens to also be restore_id's first token, or
// BackupRestoreGet stops sharing its "restore" group with BackupRestorePut.
...
func BuildCommandSpecs(ops []Operation) []CommandSpec {
	...
	for i, op := range ops {
		...
		// The OpenAPI "common" tag isn't a real resource (see the design
		// spec); its commands are ungrouped top-level commands, which this
		// package represents as an empty Tag. Task 14's emit step reads
		// that empty Tag to attach these directly to root.
		tag := op.Tag
...
func TestBuildCommandSpecs_RealSpec_MatchesDesignDocCommandTree(t *testing.T) {
	...
	want := map[string]string{ // operationId -> "tag/group.../verb"
		...
		"ResourceGet":           "connection/resource", // tagged "connection" in the real spec, not "resource" - no "resource" tag exists (confirmed via grep, see Task 11's report)
...
use a leading `/` (empty tag): `BuildCommandSpecs` itself empties `Tag` for
what was the OpenAPI `common` tag (see Step 3's implementation below) —
that's what "ungrouped top-level command" means at this layer. Task 14's
emit step consumes that already-empty `Tag` to decide root-attachment; it
doesn't do the promotion itself.
...
`backup.gen.go`, `connection.gen.go`, `schedule.gen.go`, `common.gen.go`,
and `register.gen.go` are created, each `gofmt`-clean. There is no
`resource.gen.go` — `ResourceGet` is tagged `connection` in the real spec
(not `resource`; no `resource` tag exists), so it lands in `connection.gen.go`
as `connection resource`, not a `resource list`. See Task 11's report for
